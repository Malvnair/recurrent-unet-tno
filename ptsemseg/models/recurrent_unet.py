...
def init_hidden_state(layer, prev_h, prev_state, out_size, hidden_size, batch_size, spatial_size, reference):
    """
    Init hidden state.
    :param layer:
    :param batch_size:
    ...
    if prev_h is None:
        ...
        prev_h = reference.new_ones(h_size)
        if initial == 0:
            prev_h.zero_()

    if prev_state is None:
        state_size = [batch_size, hidden_size] + list(spatial_size)
        prev_state = reference.new_ones(state_size)
        if initial == 0:
            prev_state.zero_()
...
class _RecurrentUnet(nn.Module):
    ...
    def forward(self, inputs):
        ...
        ht, _ = init_hidden_state(self, None, None, self.n_classes, self.hidden_size, batch_size, spatial_size, inputs)
        Ht = None
...
class UNetOnlyHidden(_RecurrentUnet):
    ...
    def __init__(self, args, n_classes=2, **kwargs):
        # Override this model's recurrent defaults without mutating the shared CLI namespace.
        args = copy.copy(args)
        args.hidden_size = 128
        args.structure = 'gru'

        super(UNetOnlyHidden, self).__init__(args, n_classes=n_classes, **kwargs)
        self.cell = RecurrentUNetCell(
            args,
            feature_scale=4,
            feature_level=4,
...
class _UNetWithGRU(_RecurrentUnet):
    ...
    def __init__(self, args, n_classes=2, feature_scale=4,  **kwargs):
        super(_UNetWithGRU, self).__init__(args, n_classes=n_classes, **kwargs)
...
class UNetWithGRU_R(_UNetWithGRU):
    ...
    """
    def __init__(self, args, n_classes=2, feature_scale=4, **kwargs):
        super(UNetWithGRU_R, self).__init__(args, n_classes=n_classes, feature_scale=feature_scale, **kwargs)
...
class UNetWithGRU_old(_RecurrentUnet):
    ...
    def __init__(self, args, n_classes=2, **kwargs):
        super(UNetWithGRU_old, self).__init__(args, n_classes=n_classes, **kwargs)

...
# Apex synchronized BatchNorm is intentionally not used in the CPU-first core.
...
class ConvSynbnRelu(nn.Module):
    def __init__(self, in_: int, out: int):
        super(ConvSynbnRelu, self).__init__()
        self.conv = conv3x3(in_, out)
...
class DecoderBlocknode(nn.Module):
    """
    Paramaters for Deconvolution were chosen to avoid artifacts, following
    link https://distill.pub/2016/deconv-checkerboard/
    """

    def __init__(self, low_level_inplanes, x_channels, out_channels, is_deconv=True):
        super(DecoderBlocknode, self).__init__()

        node_channels = x_channels
        in_channels = node_channels + x_channels
        middle_channels = int((in_channels + out_channels)/2)

        self.conv1 = nn.Conv2d(low_level_inplanes, x_channels, 1, bias=False)
    ...
    def forward(self, low_level, x):
        low_level_feat = self.conv1(low_level)
        x = torch.cat((low_level_feat, x), dim=1)
        return self.block(x)


class DecoderBlocknodeSynbn(nn.Module):
    """
    Paramaters for Deconvolution were chosen to avoid artifacts, following
    link https://distill.pub/2016/deconv-checkerboard/
    """
    ...
    def __init__(self, low_level_inplanes, x_channels, out_channels, is_deconv=True):
        super(DecoderBlocknodeSynbn, self).__init__()

        node_channels = x_channels
        in_channels = node_channels + x_channels
        middle_channels = int((in_channels + out_channels)/2)

        self.conv1 = nn.Conv2d(low_level_inplanes, node_channels, 1, bias=False)
        self.bn1 = BatchNorm(node_channels)
        self.relu = nn.ReLU()

        if is_deconv:
            self.block = nn.Sequential(
                ConvSynbnRelu(in_channels, middle_channels),
                nn.ConvTranspose2d(middle_channels, out_channels, kernel_size=4, stride=2,
                                   padding=1),
...
class druresnet50syncedbn_node_decbn(nn.Module):
    def __init__(self,
                 args,
                 n_classes=19,
                 initial=1,
        ...
        super(druresnet50syncedbn_node_decbn, self).__init__()
        ...
        self.encoder = build_backbone(backbone, BatchNorm, output_stride=None)
        ...
        self.dec5 = DecoderBlocknodeSynbn(2048, num_filters * 8, num_filters * 8)
        self.dec4 = DecoderBlocknodeSynbn(1024, num_filters * 8, num_filters * 8)
        self.dec3 = DecoderBlocknodeSynbn(512, num_filters * 8, num_filters * 2)
        self.dec2 = DecoderBlocknodeSynbn(256, num_filters * 2, num_filters)
        self.dec1 = DecoderBlocknodeSynbn(64, num_filters, num_filters)
    ...
    def forward(self, inputs, h, s):
        ...
        for i in range(self.steps):
            ...
            dec5 = self.dec5(conv5, center)
            dec4 = self.dec4(conv4, dec5)
...
class druresnet50syncedbn_node_nodecbn(nn.Module):
    def __init__(self,
                 args,
                 n_classes=19,
                 initial=1,
        ...
        super(druresnet50syncedbn_node_nodecbn, self).__init__()
        BatchNorm = SynchronizedBatchNorm2d
        self.args = args
        self.steps = steps
        self.feature_scale = feature_scale
        ...
        self.dec5 = DecoderBlocknode(2048, num_filters * 8, num_filters * 8)
        self.dec4 = DecoderBlocknode(1024, num_filters * 8, num_filters * 8)
        self.dec3 = DecoderBlocknode(512, num_filters * 8, num_filters * 2)
        self.dec2 = DecoderBlocknode(256, num_filters * 2, num_filters)
        self.dec1 = DecoderBlocknode(64, num_filters, num_filters)
...
class druresnet50syncedbn(nn.Module):
    def __init__(self,
                 args,
                 n_classes=19,
                 initial=1,
        ...
        super(druresnet50syncedbn, self).__init__()
        BatchNorm = SynchronizedBatchNorm2d
        self.args = args
        self.steps = steps
        self.feature_scale = feature_scale
...
class druresnet50syncedbn_decbn(nn.Module):
    def __init__(self,
                 args,
                 n_classes=19,
                 initial=1,
        ...
        super(druresnet50syncedbn_decbn, self).__init__()
        self.args = args
        self.steps = steps
        self.feature_scale = feature_scale
        self.hidden_size = hidden_size

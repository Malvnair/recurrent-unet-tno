...
class PolynomialLR(_LRScheduler):
    def __init__(self, optimizer, max_iter, decay_iter=1, 
        ...
        if decay_iter <= 0:
            raise ValueError("decay_iter must be greater than 0")
    ...
    def get_lr(self):
        effective_iter = min(
            (self.last_epoch // self.decay_iter) * self.decay_iter,
            self.max_iter,
        )
        factor = (1 - effective_iter / float(self.max_iter)) ** self.gamma
